...
- **nchar** (boolean, optional, defaults to `off`)
  ...
  on the Oracle side.  This is required if you are using a single-byte
  Oracle database character set, but have NCHAR or NVARCHAR2 columns
  containing contain characters that cannot be represented in the database
  character set.
...
- **prefetch** (optional, defaults to "200")
  ...
  Note that due to an Oracle limitation, row prefetching won't work if the
  Oracle query involves a BLOB, CLOB or BFILE column.  As a consequence, queries
  on such columns on a foreign table will perform badly if you fetch many rows.
...
Check out the [Wiki](https://github.com/laurenz/oracle_fdw/wiki) for
help with configuration on certain platforms and other problems.
Also, searching the
[GitHub issues](https://github.com/laurenz/oracle_fdw/issues)
can help - you may not be the first one with that problem.
